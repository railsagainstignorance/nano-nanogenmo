...
perl -e'@r=split(",","You_I_We_All,can_do_did_may,observe_see_spot_get_gain,a lot_much_some_lots_nowt_all_it,by_with_without_in,watching_seeing_listening_doing");sub p{my($s,$i)=@_;if($i==@r){print"$s.\n"}else{map{p("$s $_",$i+1)}split("_",$r[$i])}}p'
